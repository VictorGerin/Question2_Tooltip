...
function Tooltip(props) {
  ...
  const [windowDimensions, setWindowDimensions] = useState(getWindowDimensions()); //holds current hight and widht of the screen
  ...
  useEffect(() => { 
    ...
    switch(props.direction)
    {
      default:
      case 'top':
        if(boundingTopTip.top - boundingTip.height < 0)
          setdirection('bottom');
        else
          setdirection('top');
        break;
      case 'bottom':
        if(boundingTopTip.bottom + boundingTip.height > windowDimensions.height)
          setdirection('top');
        else
          setdirection('bottom');
        break;
      case 'right':
        ...
        break;
      case 'left':
        if(boundingTopTip.left - boundingTip.width < 0)
          setdirection('right');
        else
          setdirection('left');
        break;
  ...
  },[active, windowDimensions, props.direction, props.dissableAutoChangeDirection, props.width])
  ...
  return (
    ...
    >
      ...
      {active && (
        ...
        style={{top: position.top, left: position.left, width: props.width}} ref={divTooltip} className={`Tooltip-Tip ${direction} ${props.arrow && 'arrow'}`}>
          {props.content}
        </div>
      )}
    </div>
